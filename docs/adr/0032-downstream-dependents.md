...
- New third-party dependency in the ingestion path: libraries.io outages degrade to `null`+flag, never failed runs. Its repo↔package linkage is imperfect — polluted with junk links (handled by max-aggregation) and missing for some published repos (reads as unavailable, which understates nothing — renormalized weights — and hides nothing — flag visible).
