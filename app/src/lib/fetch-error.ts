 ...
 * mutating `/api/...` route in this app uses — see the inline
 * `NextResponse.json({ error }, { status })` calls; anything else —
 * non-object, null, missing `error`, non-string `error` — yields null so
 * the caller can fall back to its own default message.
 *
